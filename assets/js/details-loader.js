/**
 * نظام تحميل تفاصيل العقار (الإصدار النهائي v9.0 - محصّن ضد مشاكل التخزين المؤقت)
 */

// --- الدالة الجديدة: تنظيف النصوص قبل تمريرها لرابط الصورة ---
function cleanTextForImage(text) {
    if (typeof text !== 'string') return '';
    return text
        .replace(/م²/g, 'متر')
        .replace(/,/g, '')
        .replace(/جنيه/g, 'ج')
        .replace(/[^\u0600-\u06FFa-zA-Z0-9\s]/g, '') // إزالة الرموز الغريبة
        .trim();
}
...
// --- الدالة المحصّنة لتحديث وسوم السيو ---
function updateSeoTags(prop, propertyId) {
    ...
    // تنظيف النصوص
    const imageTitle = cleanTextForImage(prop.title || 'عرض عقاري مميز');
    const imagePrice = cleanTextForImage(prop.price_clean || prop.price_display || '');
    const imageArea = cleanTextForImage(prop.area_clean || prop.area_display || '');

    // ترميز النصوص
    const encodedTitle = encodeURIComponent(imageTitle.substring(0, 60));
    const encodedPrice = encodeURIComponent(imagePrice);
    const encodedArea = encodeURIComponent(imageArea);

    // رابط الصورة مع Cache Buster
    const cacheBuster = `&v=${Date.now()}`;
    const autoShareImage = `/.netlify/functions/og-image?title=${encodedTitle}&price=${encodedPrice}&area=${encodedArea}${cacheBuster}`;
...
function renderPropertyDetails(prop, container, propertyId) {
    ...
    container.innerHTML = `
        ...
        <footer class="details-actions">
            ...
            <button onclick="copyToClipboard('${pageURL}')" class="action-btn copy-btn" title="انسخ رابط العرض">📤</button>
            <a href="/properties-filtered.html" class="action-btn back-btn">← العودة للقائمة</a>
        </footer>
