/**
 * نظام تحميل تفاصيل العقار (الإصدار النهائي v8.2 - مع دمج نظام القصص الذكي)
 */
...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  try {
    ...
    // استدعاء الدوال الكاملة والنهائية (مع نظام القصص)
    await updateSeoTagsWithStory(propertyData, propertyId); 
...
// --- الجزء الثاني: الدوال النهائية والاحترافية مع الإصلاحات ---
...
// ✨✨✨ تم دمج نظام القصص الذكي هنا (النسخة المصححة) ✨✨✨
async function updateSeoTagsWithStory(prop, propertyId) {
  const pageTitle = `${prop.title || 'عرض عقاري'} - سمسar طلبك`;
  const defaultDescription = `تفاصيل عقار: ${prop.title || ''}. ${(prop.summary || prop.description || '').substring(0, 160)}...`;
  const pageURL = new URL(`/property/${propertyId}`, window.location.origin).href;

  let shareImage = '/assets/img/default-share-image.jpg'; // تأكد من وجود صورة افتراضية بهذا المسار
  let shareText = defaultDescription;

  if (prop.story) {
    try {
      // ✨ تم تصحيح المسار ليقرأ من ملف JSON مباشرة
      const response = await fetch('/data/stories.json?t=' + Date.now());
      if (response.ok) {
        const stories = await response.json();
        const selectedStory = stories.find(s => s.name === prop.story);
        if (selectedStory) {
          shareImage = selectedStory.image;
          shareText = selectedStory.text;
        }
      }
    } catch (e) {
      console.error("فشل في جلب مكتبة القصص:", e);
    }
  }
  ...
  document.querySelector('meta[property="og:description"]')?.setAttribute('content', shareText); // استخدام نص القصة هنا
  ...
  ogImageMeta.setAttribute('content', shareImage); // استخدام صورة القصة هنا
...
// دالة العرض تبقى كما هي من نسختك الناجحة
...
**الخطوات النهائية:**
1.  استبدل محتوى ملف `details-loader.js` بهذا الكود الكامل.
2.  قم بنفس التعديل على ملف `requests-details-script.js` (استبدل دالة `updateSeoTags` بالنسخة الجديدة `updateSeoTagsWithStory` وعدّل الاستدعاء).
3.  ارفع التغييرات.

بهذا نكون قد أصلحنا آخر نقطة فنية. أنا واثق أن النظام سيعمل بشكل مثالي هذه المرة.
