/**
 * نظام تحميل تفاصيل العقار (الإصدار النهائي v8.6 - مع إصلاح التخزين المؤقت للصور)
 */
...
// 👇👇👇 هذه هي الدالة الوحيدة التي تم تحديثها بالإصلاح النهائي 👇👇👇
function updateSeoTags(prop, propertyId) {
  ...
  // 2. بناء رابط القالب بشكل صريح
  const templateUrl = `https://aqarnasr.netlify.app/og-image-template.html`;

  // 3. إضافة متغير عشوائي (cache buster) لضمان تحديث الصورة دائمًا
  const cacheBuster = `&v=${new Date().getTime()}`;
  
  // 4. بناء رابط الصورة التلقائي النهائي
  const autoShareImage = `https://og-image.vercel.app/?template=${encodeURIComponent(templateUrl)}&title=${imageTitle}&price=${imagePrice}&area=${imageArea}${cacheBuster}`;
  
  // 5. استخدام الصورة التلقائية، أو استخدام صورة مخصصة إذا أضفت حقل "share_image" في الـ JSON
