/**
 * نظام تحميل تفاصيل العقار (الإصدار النهائي v8.7 - محصّن ضد الأخطاء)
 */

// --- الدالة الجديدة والمهمة ---
function cleanTextForImage(text) {
    if (typeof text !== 'string') return '';
    // إزالة الرموز الخاصة والاقتصار على الأساسيات
    return text.replace(/م²/g, 'متر').replace(/,/g, '').replace(/جنيه/g, 'ج');
}
...
// --- الجزء الثاني: الدوال النهائية والاحترافية مع الإصلاح ---
...
function updateSeoTags(prop, propertyId) {
  ...
  // 1. استخدام البيانات النظيفة إن وجدت، أو تنظيف البيانات العادية
  ...
  const templateUrl = `https://aqarnasr.netlify.app/og-image-template.html`;
  const cacheBuster = `&v=${new Date().getTime()}`;
  const autoShareImage = `https://og-image.vercel.app/?template=${encodeURIComponent(templateUrl)}&title=${encodedTitle}&price=${encodedPrice}&area=${encodedArea}${cacheBuster}`;
