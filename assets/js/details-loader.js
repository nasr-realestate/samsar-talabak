/**
 * نظام تحميل تفاصيل العقار (الإصدار النهائي v8.5 - مع إصلاح رابط الصور التلقائية)
 */
...
function updateSeoTags(prop, propertyId) {
  ...
  // 2. بناء رابط الصورة التلقائي (النسخة الأكثر توافقًا)
  // نحن نخبر الخدمة بشكل صريح أين تجد القالب الخاص بنا
  const templateUrl = `https://aqarnasr.netlify.app/og-image-template.html`;
  const autoShareImage = `https://og-image.vercel.app/?template=${encodeURIComponent(templateUrl)}&title=${imageTitle}&price=${imagePrice}&area=${imageArea}`;
