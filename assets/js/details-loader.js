/**
 * 🏢 سمسار طلبك - نظام عرض العقارات (النسخة النهائية الكاملة v4.0 - تصميم أصلي ومنطق محدث)
 */
...
class EnhancedPropertyDisplay {
  constructor() {
    ...
    this.categories = {
      "apartments": { label: "🏠 شقق للبيع", icon: "🏠" },
      "apartments-rent": { label: "🏡 شقق للإيجار", icon: "🏡" },
      "shops": { label: "🏪 محلات تجارية", icon: "🏪" },
      "offices": { label: "🏢 مكاتب إدارية", icon: "🏢" },
      "admin-hq": { label: "🏛️ مقرات إدارية", icon: "🏛️" }
    };
  ...
  async handleDateFilterChange(newFilterValue) {
    ...
    if(this.currentCategory) {
        await this.loadCategory(this.currentCategory, true); // Force refresh to apply sorting
    }
  }
  ...
  displayProperties(properties) {
    ...
    filteredProperties.forEach((property) => {
      const card = this.createPropertyCard(property, this.currentCategory);
    ...
    card.innerHTML = `
        <div class="property-header">
            <div class="property-brand">
                سمسار طلبك
                <span class="property-category-badge" style="background-color: ${categoryInfo.color}; color: black; font-weight: bold;">
