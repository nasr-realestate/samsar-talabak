/**
 * 🏢 سمسار طلبك - نظام عرض العقارات المحسن (النسخة النهائية الكاملة v2.1)
 * تم تحديثه ليدعم القوام الجديد والقديم لملفات JSON في عرض القوائم.
 */
...
class EnhancedPropertyDisplay {
  ...
  createPropertyCard(property, category, index) {
    ...
    // ✨ الإصلاح: منطق ذكي لتحديد القيم التي ستظهر في البطاقة
  ...
  async shareProperty(property) {
    const detailPageUrl = new URL(`/samsar-talabak/details.html?category=${property.category}&file=${encodeURIComponent(property.filename)}`, window.location.origin);
...
if (!document.querySelector('style[id="enhanced-display-styles"]')) {
    const styleElement = document.createElement('style');
    styleElement.id = 'enhanced-display-styles';
