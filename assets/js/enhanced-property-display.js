/**
 * 🏢 سمسار طلبك - نظام عرض العقارات (النسخة النهائية - مطابقة لمنطق الطلبات الناجح)
 */
...
class EnhancedPropertyDisplay {
  constructor() {
    ...
    this.categories = {
      "apartments": { label: "🏠 شقق للبيع", icon: "🏠", color: "#00ff88", description: "شقق سكنية فاخرة", folder: "apartments" },
      "apartments-rent": { label: "🏡 شقق للإيجار", icon: "🏡", color: "#00ccff", description: "شقق للإيجار الشهري", folder: "apartments-rent" },
      "shops": { label: "🏪 محلات تجارية", icon: "🏪", color: "#ff6b35", description: "محلات ومساحات تجارية", folder: "shops" },
      "offices": { label: "🏢 مكاتب إدارية", icon: "🏢", color: "#8b5cf6", description: "مكاتب ومساحات عمل", folder: "offices" },
      "admin-hq": { label: "🏛️ مقرات إدارية", icon: "🏛️", color: "#f59e0b", description: "مقرات ومباني إدارية", folder: "admin-hq" }
    };
  ...
  handleWelcomeMessage() {
    ...
    const hasShownWelcome = localStorage.getItem("welcomeShown_properties");
    if (!hasShownWelcome) {
  ...
  hideWelcomeMessage() {
    ...
    setTimeout(() => {
      ...
      localStorage.setItem("welcomeShown_properties", "true");
    }, 400);
  }
  ...
  async handleCategoryChange(category, button) {
    ...
    try {
      ...
      localStorage.setItem('lastCategory_properties', category);
  ...
  loadDefaultCategory() {
    const savedCategory = localStorage.getItem('lastCategory_properties');
  ...
  async fetchCategoryData(category) {
    ...
    while (retries < this.config.maxRetries) {
      try {
        ...
        const indexPath = `/data/properties/${folderName}/index.json?t=${Date.now()}`;
        const indexResponse = await fetch(indexPath);
        if (!indexResponse.ok) throw new Error(`HTTP ${indexResponse.status}`);
        const files = await indexResponse.json();
        if (!Array.isArray(files) || files.length === 0) return [];
        const propertyPromises = files.map(filename => this.fetchPropertyData(folderName, filename, category));
  ...
  async fetchPropertyData(folderName, filename, category) {
    try {
      const response = await fetch(`/data/properties/${folderName}/${filename}?t=${Date.now()}`);
  ...
  setupCardEvents(card, property) {
    ...
    viewDetailsBtn.addEventListener('click', () => { localStorage.setItem('lastViewedCard_properties', property.filename); });
  ...
  setupCardInteractions() {
    const highlightedCardFile = localStorage.getItem('highlightCard_properties');
  ...
  highlightLastViewedCard() {
    const lastViewedFilename = localStorage.getItem('lastViewedCard_properties');
    ...
    if (cardToHighlight) {
      ...
      localStorage.removeItem('lastViewedCard_properties');
    }
  }
  ...
  addFavorite(property) {
    ...
    if (!favorites.includes(property.filename)) {
      ...
      localStorage.setItem('favorites_properties', JSON.stringify(favorites));
    }
  }
  ...
  getFavorites() {
    try { return JSON.parse(localStorage.getItem('favorites_properties') || '[]'); } catch { return []; }
  }

  injectStyles() {
    const styleId = 'enhanced-properties-styles';
    if (document.getElementById(styleId)) return;
    
    const additionalStyles = `<style id="${styleId}">
      /* ... (Paste your full CSS here if needed) ... */
    </style>`;
