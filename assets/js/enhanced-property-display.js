/**
 * 🏢 سمسار طلبك - نظام عرض العقارات المحسن (النسخة النهائية المحسنة للـ SEO)
 * Enhanced Property Display System (SEO Optimized Full Version)
 * 
 * الميزات المدمجة:
 * - دعم كامل لبيانات Schema.org (JSON-LD)
 * - تحسينات SEO متقدمة للعقارات
 ...
 * - فلتر للفرز حسب تاريخ الإضافة
 * - حل مشكلة التحديث التلقائي عند التمرير
 * - ✨ عرض ملخص في البطاقة وتفاصيل أوفى في صفحة التفاصيل
 */
...
class EnhancedPropertyDisplay {
  ...
  createFilterButton(key, category, index) {
    ...
    button.setAttribute('aria-label', `تصفية حسب ${category.label}`);
  ...
  createDateFilter() {
    ...
    filterWrapper.innerHTML = `
      ...
      <select id="date-filter" class="date-filter-select" aria-label="اختر طريقة الفرز حسب التاريخ">
  ...
  async displayProperties(properties, category) {
    ...
    this.generateStructuredData(filteredProperties, category);
  }

  // إنشاء بيانات منظمة JSON-LD للعقارات
  generateStructuredData(properties, category) {
    const structuredData = {
      ...
      "itemListElement": properties.map((property, index) => ({
        "@type": "ListItem",
        "position": index + 1,
        "item": this.generatePropertyStructuredData(property)
      }))
    };
  ...
  // إنشاء بيانات منظمة لكل عقار
  generatePropertyStructuredData(property) {
    const categoryInfo = this.categories[property.category] || {};
    const detailPage = `/samsar-talabak/details.html?category=${property.category}&file=${encodeURIComponent(property.filename)}`;
    ...
    return {
      ...
      "image": "https://i.postimg.cc/Vk8Nn1xZ/me.jpg",
      "offers": {
        "@type": "Offer",
  ...
  createPropertyCard(property, category, index) {
    ...
    card.setAttribute('aria-label', `بطاقة عقار: ${property.title}`);
    card.style.cssText = `
    ...
    // ✨ التعديل الأهم: استخدام الملخص بدلاً من الوصف الكامل
    const descriptionText = property.summary || property.description;

    card.innerHTML = `
      <div class="property-header">
        <img src="https://i.postimg.cc/Vk8Nn1xZ/me.jpg" alt="شعار سمسار طلبك" class="property-logo" loading="lazy" width="50" height="50">
        ...
        <div class="property-actions">
          <button class="favorite-btn" title="إضافة للمفضلة" aria-label="إضافة العقار للمفضلة"><span class="heart-icon">♡</span></button>
          <button class="share-btn" title="مشاركة" aria-label="مشاركة العقار"><span class="share-icon">📤</span></button>
        </div>
      </div>
      ...
      <div class="property-footer">
        <a href="${detailPage}" class="view-details-btn" aria-label="عرض تفاصيل ${property.title}"><span class="btn-icon">👁️</span><span class="btn-text">عرض التفاصيل الكاملة</span><span class="btn-arrow">←</span></a>
  ...
  async shareProperty(property) {
    ...
    const shareData = { 
      ...
      text: `شاهد هذا العقار المميز: ${property.title} - ${property.price}`, 
      url: window.location.origin + detailPage 
    };
    try {
      if (navigator.share) {
