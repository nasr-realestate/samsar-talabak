...
class EnhancedPropertyDisplay {
  ...
  /**
   * معالجة إيماءات السحب (النسخة المعدّلة)
   */
  handleSwipeGesture() {
    const swipeThreshold = 50;
    const diff = this.touchStartY - this.touchEndY;

    // ✨ التحسين: تم تعطيل "السحب لأعلى للتحديث" لمنع إعادة التحميل التلقائي المزعج.
    // تم الإبقاء فقط على ميزة "السحب لأسفل للعودة إلى الأعلى" لتحسين تجربة المستخدم.
    if (diff < -swipeThreshold) { // diff < 0 يعني سحب لأسفل
