/**
 * 🏢 سمسار طلبك - نظام عرض العقارات المحسن (النسخة النهائية الكاملة v3.2 - إصلاح الروابط)
 */
...
class EnhancedPropertyDisplay {
  ...
  createPropertyCard(property, category, index) {
    ...
    // ✨ الإصلاح الحاسم 1: ضمان وجود ID دائماً، باستخدام اسم الملف كخيار احتياطي.
    // هذا سيصلح مشكلة الروابط المعطلة إذا كان حقل "id" غير موجود في ملف الـ JSON.
    const propertyId = property.id || (property.filename ? property.filename.replace('.json', '') : '');
  ...
  handleCardClick(card, property) {
    // ✨ الإصلاح الحاسم 2: ضمان توجيه المستخدم إلى رابط صحيح حتى لو تم الضغط على البطاقة نفسها.
    const propertyId = property.id || (property.filename ? property.filename.replace('.json', '') : '');
  ...
  async shareProperty(property) {
    // ✨ الإصلاح الحاسم 3: التأكد من أن رابط المشاركة يحتوي على ID صحيح.
    const propertyId = property.id || (property.filename ? property.filename.replace('.json', '') : '');
